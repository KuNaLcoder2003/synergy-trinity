...
const FIELDS: FieldMeta[] = [
    { key: "company_name", label: "Company Name", placeholder: "e.g. Apex Traders Pvt Ltd", icon: <BuildingIcon />, colSpan: "full" },
    { key: "name", label: "Contact Name", placeholder: "e.g. Rohan Mehta", icon: <UserIcon />, colSpan: "half" },
    { key: "mobile", label: "Mobile Number", placeholder: "+91 98201 XXXXX", icon: <PhoneIcon />, type: "tel", colSpan: "half" },
    { key: "email", label: "Email Address", placeholder: "rohan@company.com", icon: <MailIcon />, type: "email", colSpan: "full" },
    ...
    { key: "upi_id", label: "UPI ID", placeholder: "name@bankname", icon: <UpiIcon />, colSpan: "half" },
    { key: "pincode", label: "Pincode", placeholder: "e.g. 400001", icon: <HashIcon />, colSpan: "half" },
    { key: "state", label: "State", placeholder: "e.g. Maharashtra", icon: <MapIcon />, colSpan: "half" },
    { key: "country", label: "Country", placeholder: "e.g. India", icon: <GlobeIcon />, colSpan: "half" },
];

const EMPTY_FORM: CustomerForm = {
    company_name: "", name: "", mobile: "", email: "",
    bank_details: "", upi_id: "", state: "", country: "India", pincode: "",
};
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function AddCustomer({ onClose, onSubmit }: AddCustomerProps) {
    ...
    const handleChange = (key: keyof CustomerForm, val: string) => {
    ...
    return (
      ...
      `}</style>
            ...
            {/* Overlay */}
            <div
                className="overlay-bg fixed inset-0 z-50 flex items-center justify-center p-4"
                style={{ backgroundColor: "rgba(0,0,0,0.3)", backdropFilter: "blur(8px)" }}
                onClick={(e) => e.target === e.currentTarget && onClose()}
            >
                {/* Modal card */}
                <div
                    className="modal-card relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col"
                ...
                >
                    ...
                    {success && (
                        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center rounded-3xl bg-white">
                            <div className="success-icon flex items-center justify-center w-20 h-20 rounded-full bg-emerald-50 mb-4">
                            ...
                            <p className="text-xl font-bold text-gray-900 mb-1">Customer Added!</p>
                    ...
                    <div className="flex items-center justify-between px-7 pt-6 pb-5 border-b border-gray-100">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-base font-bold text-gray-900 leading-tight">New Customer</h2>
                                <p className="text-xs text-gray-400 mt-0.5">Fill in the details to onboard a customer</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
