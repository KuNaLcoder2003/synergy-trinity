...
import type { Material, Docs, Orders } from "../types"; // adjust path
...
const CreateOrderForm: React.FC<CreateOrderFormProps> = ({ onClose }) => {
    ...
    const handleDocsSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.append("docs_meta", JSON.stringify(docMeta));
            const res = await fetch(`${BACKEND_URL}/order/${orderId}/docs`, {
                method: "POST",
                body: formData,
            });
            if (!res.ok) throw new Error(`Server error: ${res.status}`);
                    ...
                    {activeStep === 1 && (
                        <form id="order-form" onSubmit={handleOrderSubmit} className="space-y-5">
                            ...
                            <div className="bg-gray-50 rounded-2xl border border-gray-100 p-5">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <LabeledInput label="Supplier ID" required placeholder="MongoDB ObjectId"
                                        value={form.supplier_id} onChange={e => setField("supplier_id", e.target.value)} />
                                    <LabeledInput label="Customer ID" required placeholder="MongoDB ObjectId"
                                        value={form.customer_id} onChange={e => setField("customer_id", e.target.value)} />
                                </div>
                            </div>
